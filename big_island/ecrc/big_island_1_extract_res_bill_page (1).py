...
def text_matches(page_text: str) -> bool:
    ...
    norm  = normalize(upper)   # whitespace-collapsed version for robust matching
    ...
    if SEARCH_TEXT in norm:
        return True

    # Strategy 3: core keywords present (drop AVERAGE — sometimes missing from extraction)
    if all(kw in upper for kw in ["CALCULATIONS", "RESIDENTIAL", "CUSTOMER", "BILL"]):
        return True

    # Strategy 4 & 5: HELCO PDFs label this page "ATTACHMENT 9" (newer) or
    # "ATTACHMENT 10" (older filings). PyPDF2 sometimes inserts extra spaces
    # (e.g. "ATTACHMENT  10"), so match on the normalized string.
    # Require both 500 KWH and 600 KWH tables to avoid false hits on the
    # ATTACHMENT 9A fuel-factor history page.
    helco_keywords = ["500 KWH", "600 KWH", "RESIDENTIAL", "CUSTOMER"]
    if "ATTACHMENT 9" in norm and all(kw in upper for kw in helco_keywords):
        return True
    if "ATTACHMENT 10" in norm and all(kw in upper for kw in helco_keywords):
        return True
